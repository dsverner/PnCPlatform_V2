...
import { openOrDownload } from '@/lib/files'
  ...
  return (
    <div id="files-panel"><Panel title={`Files and records · ${q.isPending ? '…' : rows.length}`}>
      ...
      <DataGrid rows={rows} rowKey={(x, ) => s(x.fileRowId) || s(x.what) + s(x.name) + s(x.when)} columns={[{ key: 'what', label: 'Record' }, { key: 'kind', label: 'Kind' },
        { key: 'name', label: 'File / summary', render: (x) => (<>{x.fileRowId ? <button type="button" className="text-sky-300 underline" onClick={() => void openOrDownload(s(x.fileRowId), s(x.name), s(x.mime)).catch((e) => setFileErr(e instanceof Error ? e.message : String(e)))}>{s(x.name)}</button> : s(x.name)}{noteOf(x.note) && <span className="ml-2 text-xs text-slate-400">{noteOf(x.note)}</span>}</>) },
      ...
      <Status>A file name opens the file — a PDF or a text in its own tab, a Word document or a settings file saved by the browser (every open is a logged read, #144). A legacy rationale is frozen at its revision (#217); from the next change on a device the rationale is generated by the platform.</Status>
    </Panel></div>
  )
}
