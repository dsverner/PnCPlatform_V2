...
CREATE PROCEDURE [asset].[InstrumentWinding_Revise]
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM [asset].[InstrumentWinding] WHERE [EntityId] = @EntityId AND [IsDeleted] = 0 AND [ValidTo] IS NULL)
        THROW 50103, N'asset.InstrumentWinding: no open row for @EntityId to revise.', 1;
    BEGIN TRANSACTION;
