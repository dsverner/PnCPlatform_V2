-- SCHEMA-DESIGN §0.4 (68). The identities the field and the drawings use for a asset.
-- The design writes EntityId as "the thing the key names"; here each key row has its own identity
-- (EntityId → AlternateKeyRegistry) and SubjectEntityId names the asset, because the base
-- rule of one current row per EntityId would otherwise allow only one key per thing (STEPS.md).
CREATE TABLE [asset].[AlternateKey] (
    [RowSeq]            BIGINT IDENTITY(1,1) NOT NULL,
    [RowId]             UNIQUEIDENTIFIER  NOT NULL CONSTRAINT [DF_AlternateKey_RowId] DEFAULT NEWSEQUENTIALID(),
    [EntityId]          UNIQUEIDENTIFIER  NOT NULL CONSTRAINT [FK_AlternateKey_Registry] REFERENCES [asset].[AlternateKeyRegistry] ([EntityId]),
    ...
    [SubjectEntityId]    UNIQUEIDENTIFIER NOT NULL CONSTRAINT [FK_AlternateKey_Subject] REFERENCES [asset].[AssetRegistry] ([EntityId]),
...
) WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [asset].[AlternateKey_History]));
GO
CREATE INDEX [IX_AlternateKey_Entity] ON [asset].[AlternateKey] ([EntityId], [ValidFrom]);
GO
CREATE UNIQUE INDEX [UX_AlternateKey_Value] ON [asset].[AlternateKey] ([KeyKindCode], [KeyValue], [ScopeEntityId]) WHERE [ValidTo] IS NULL AND [IsDeleted] = 0;
GO
CREATE INDEX [IX_AlternateKey_Subject] ON [asset].[AlternateKey] ([SubjectEntityId]) WHERE [ValidTo] IS NULL AND [IsDeleted] = 0;
GO
EXEC sys.sp_addextendedproperty @name = N'PnC.TemporalClass', @value = N'ValidTime',
    @level0type = N'SCHEMA', @level0name = N'asset', @level1type = N'TABLE', @level1name = N'AlternateKey';
GO
