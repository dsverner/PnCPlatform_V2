-- SCHEMA-DESIGN §0.4 (68). The identities an external system uses for a layer node (LayerNodeNumber, scoped to its layer).
-- Each key row has its own identity (EntityId → AlternateKeyRegistry) and SubjectEntityId names the
-- layer node (see STEPS.md step 3 flag).
CREATE TABLE [network].[AlternateKey] (
    [RowSeq]            BIGINT IDENTITY(1,1) NOT NULL,
    [RowId]             UNIQUEIDENTIFIER  NOT NULL CONSTRAINT [DF_AlternateKey_RowId] DEFAULT NEWSEQUENTIALID(),
    [EntityId]          UNIQUEIDENTIFIER  NOT NULL CONSTRAINT [FK_AlternateKey_Registry] REFERENCES [network].[AlternateKeyRegistry] ([EntityId]),
    ...
    [SubjectEntityId]    UNIQUEIDENTIFIER NOT NULL CONSTRAINT [FK_AlternateKey_Subject] REFERENCES [network].[LayerNodeRegistry] ([EntityId]),
...
) WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [network].[AlternateKey_History]));
GO
CREATE INDEX [IX_AlternateKey_Entity] ON [network].[AlternateKey] ([EntityId], [ValidFrom]);
GO
CREATE UNIQUE INDEX [UX_AlternateKey_Value] ON [network].[AlternateKey] ([KeyKindCode], [KeyValue], [ScopeEntityId]) WHERE [ValidTo] IS NULL AND [IsDeleted] = 0;
GO
CREATE INDEX [IX_AlternateKey_Subject] ON [network].[AlternateKey] ([SubjectEntityId]) WHERE [ValidTo] IS NULL AND [IsDeleted] = 0;
GO
EXEC sys.sp_addextendedproperty @name = N'PnC.TemporalClass', @value = N'ValidTime',
    @level0type = N'SCHEMA', @level0name = N'network', @level1type = N'TABLE', @level1name = N'AlternateKey';
GO
