-- SCHEMA-DESIGN §0.4 (68). The identities the field and the drawings use for a scheme.
-- Each key row has its own identity (EntityId → AlternateKeyRegistry) and SubjectEntityId names the
-- scheme (see STEPS.md step 3 flag).
CREATE TABLE [scheme].[AlternateKey] (
    [RowSeq]            BIGINT IDENTITY(1,1) NOT NULL,
    [RowId]             UNIQUEIDENTIFIER  NOT NULL CONSTRAINT [DF_AlternateKey_RowId] DEFAULT NEWSEQUENTIALID(),
    [EntityId]          UNIQUEIDENTIFIER  NOT NULL CONSTRAINT [FK_AlternateKey_Registry] REFERENCES [scheme].[AlternateKeyRegistry] ([EntityId]),
    ...
    [SubjectEntityId]    UNIQUEIDENTIFIER NOT NULL CONSTRAINT [FK_AlternateKey_Subject] REFERENCES [scheme].[SchemeRegistry] ([EntityId]),
...
) WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [scheme].[AlternateKey_History]));
GO
CREATE INDEX [IX_AlternateKey_Entity] ON [scheme].[AlternateKey] ([EntityId], [ValidFrom]);
GO
CREATE UNIQUE INDEX [UX_AlternateKey_Value] ON [scheme].[AlternateKey] ([KeyKindCode], [KeyValue], [ScopeEntityId]) WHERE [ValidTo] IS NULL AND [IsDeleted] = 0;
GO
CREATE INDEX [IX_AlternateKey_Subject] ON [scheme].[AlternateKey] ([SubjectEntityId]) WHERE [ValidTo] IS NULL AND [IsDeleted] = 0;
GO
EXEC sys.sp_addextendedproperty @name = N'PnC.TemporalClass', @value = N'ValidTime',
    @level0type = N'SCHEMA', @level0name = N'scheme', @level1type = N'TABLE', @level1name = N'AlternateKey';
GO
