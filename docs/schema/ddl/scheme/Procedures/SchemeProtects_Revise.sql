...
CREATE PROCEDURE [scheme].[SchemeProtects_Revise]
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM [scheme].[SchemeProtects] WHERE [EntityId] = @EntityId AND [IsDeleted] = 0 AND [ValidTo] IS NULL)
        THROW 50103, N'scheme.SchemeProtects: no open row for @EntityId to revise.', 1;
    BEGIN TRANSACTION;
    -- close the prior fact (decision 71: end of validity is a valid-time update)
    UPDATE [scheme].[SchemeProtects] SET [ValidTo] = @ValidFrom, [ModifiedBy] = @ActorId, [ModifiedAt] = @now
