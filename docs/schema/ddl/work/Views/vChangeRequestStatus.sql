...
OUTER APPLY (SELECT TOP (1) p.[NodeEntityId] FROM [asset].[vPlacement] p WHERE p.[AssetEntityId] = sa.[EntityId] AND p.[PlacementKind] = N'Installed' ORDER BY p.[ValidFrom] DESC) spl
...
OUTER APPLY (SELECT TOP (1) i.[EntityId], i.[CurrentState], i.[StartedAt], i.[CompletedAt], i.[IsCancelled], i.[WorkflowDefinitionVersionRowId]
             FROM [process].[vWorkflowInstance] i WHERE i.[SubjectKind] = N'WorkRequest' AND i.[SubjectEntityId] = w.[EntityId]
...
OUTER APPLY (SELECT TOP (1) p.[EntityId], p.[State], p.[Outcome], p.[StartedAt], p.[CompletedAt],
             ...
             FROM [process].[vProcedureInstance] p
...
OUTER APPLY (SELECT TOP (1) b.[State], b.[Outcome], b.[CompletedAt] FROM [process].[vBlockInstance] b
             WHERE b.[ProcedureInstanceEntityId] = pi.[EntityId] AND b.[BlockPath] = N'MAIN/COMPLETION/DOCUMENTATION' ORDER BY b.[RowSeq] DESC) doc
OUTER APPLY (SELECT TOP (1) b.[State], b.[Outcome], b.[CompletedAt] FROM [process].[vBlockInstance] b
             WHERE b.[ProcedureInstanceEntityId] = pi.[EntityId] AND b.[BlockPath] = N'MAIN/COMPLETION/DATABASE' ORDER BY b.[RowSeq] DESC) db
OUTER APPLY (SELECT TOP (1) c.[EntityId] FROM [process].[vProcedureInstance] c
             WHERE c.[ParentInstanceEntityId] = pi.[EntityId] AND c.[CallBlockPath] = N'MAIN/COMPLETION/DOCUMENTATION/UPDATE_DRAWINGS' ORDER BY c.[StartedAt] DESC) child
OUTER APPLY (SELECT TOP (1) s.[Draft], s.[CommittedRecordEntityId], s.[CommittedAt]
             ...
             WHERE b.[ProcedureInstanceEntityId] = child.[EntityId] AND s.[StepId] = N'RECORD_REVISION' AND s.[State] = N'Committed' ORDER BY s.[CommittedAt] DESC) rr
OUTER APPLY (SELECT TOP (1) s.[Outcome]
             FROM [process].[vBlockInstance] b JOIN [process].[vStepInstance] s ON s.[BlockInstanceEntityId] = b.[EntityId]
             WHERE b.[ProcedureInstanceEntityId] = child.[EntityId] AND s.[StepId] = N'IDENTIFY_DRAWINGS' AND s.[State] = N'Committed' ORDER BY s.[CommittedAt] DESC) idn
OUTER APPLY (SELECT TOP (1) s.[EntityId] AS [StepInstanceEntityId], s.[State], s.[CommittedRecordEntityId]
             FROM [process].[vBlockInstance] b JOIN [process].[vStepInstance] s ON s.[BlockInstanceEntityId] = b.[EntityId]
             WHERE b.[ProcedureInstanceEntityId] = pi.[EntityId] AND s.[StepId] = N'BASELINE' ORDER BY s.[CommittedAt] DESC, s.[RowSeq] DESC) bl
OUTER APPLY (SELECT TOP (1) s.[EntityId] AS [StepInstanceEntityId], s.[State], s.[CommittedAt]
             FROM [process].[vBlockInstance] b JOIN [process].[vStepInstance] s ON s.[BlockInstanceEntityId] = b.[EntityId]
             WHERE b.[ProcedureInstanceEntityId] = pi.[EntityId] AND s.[StepId] = N'RETURN_TO_SERVICE' ORDER BY s.[CommittedAt] DESC, s.[RowSeq] DESC) rts;
GO
