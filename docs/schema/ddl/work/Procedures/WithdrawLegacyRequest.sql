...
-- and stays on the device's History), and the request is cancelled through the workflow's own Cancel, which cancels the
-- landed procedure with the reason (#228), whose role rule and reason rule still apply. The in-service settings are untouched,
...
BEGIN
    ...
    -- #228: the workflow's Cancel cancels the landed run (and any child run) with this reason — process.Transition's cascade
