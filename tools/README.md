...
| `ot/gate_accounts.py` | the three gate accounts on `vgsot.internal` through the Samba DC (decision #97); idempotent; passwords generated here and written only to `dev.local` |
| `ot/gate_runs.py` | the Windows-mode API smoke on VGS-VM02 as each gate account — a one-shot scheduled task per account, run, read back, deleted (`docs/runbook/VM02-SECOND-SITE.md`) |
